    ...
    public class host_handler extends Thread{
        ...
        public void run(){
            try{
                ...
                System.err.println("Could not get the host credentials. Error3: " + e.getMessage());
        ...
        private ArrayList<String> get_host_creds() throws Exception{
            ...
            ArrayList<String> host_cred = new ArrayList<String>(){{
                try{
                    ...
                    System.err.println("Could not get the host credentials. Error2: " + e.getMessage());
                    throw new Exception();
                }
            };};
        ...
        private void keyword_search(String keyword) throws Exception{
            ...
            try {
                ...
                ArrayList<Integer> file_keys = new ArrayList<Integer>(Collections.list(file_info.keys()));

                for (Integer info : file_keys) {
                    for(String in : file_info.get(info)){
                        if(in.contains(keyword)){
                            // return to client hostname port filename connSpeed

                            String hostname = creds.get(info).get(1); // creds.get(i).get(1) => hostname
                            String port = creds.get(info).get(3);
                            String filename = in; // file_info.get(i).get(0) => filename
                            String connSpeed = creds.get(info).get(2); // creds.get(i).get(2) => connSpeed
        ...
        private void print_tables(){
            ...
            ArrayList<Integer> creds_keys = new ArrayList<Integer>(Collections.list(creds.keys()));

            for (Integer cred : creds_keys) {
                for(String info : creds.get(cred)){
                    System.out.println("creds["+cred+"]: " + info);
                }
            }
            ...
            for (Integer info : file_keys) {
                for(String in : file_info.get(info)){
                    System.out.println("creds["+info+"]: " + in);
                }
            }
            // for(Integer cred_key : creds.keys()){
